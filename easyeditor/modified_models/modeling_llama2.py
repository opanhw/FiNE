...
from transformers.models.llama.modeling_llama import LlamaModel, LlamaPreTrainedModel, LLAMA_INPUTS_DOCSTRING, _CONFIG_FOR_DOC
from torch.utils.checkpoint import checkpoint

class _LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def set_get_scores(self, start=None, end=None):
        self.scores = []
        self.cal_score = True
        self.start = start
        self.end = end

    def set_get_new_scores(self, start=None, end=None):
        self.new_scores = []
    ...
    def forward(
        ...
        if self.cal_activations:
            self.activations = [(self.act[n][0].detach()[:]).half().cpu() for n in
                               range(self.config.num_hidden_layers)]
            last_activation = [(self.act[n][0].detach()[:, -1]).half().cpu() for n in
                               range(self.config.num_hidden_layers)]
